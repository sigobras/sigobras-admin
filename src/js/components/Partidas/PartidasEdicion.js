...
import { MdFlashOn, MdSearch, MdChevronLeft, MdChevronRight, MdVisibility, MdMonetizationOn, MdWatch, MdLibraryBooks, MdCancel, MdArrowDropDownCircle, MdAddAPhoto, MdClose, MdModeEdit, MdSave } from 'react-icons/md';
...
export default () => {
	...
	const [InputActividadMetradoEstado, setInputActividadMetradoEstado] = useState(-1);
	function toggleInputActividadMetradoEstado(actividad){
		setInputActividadMetradoEstado(actividad)
	}
	const [InputActividadMetradoValor, setInputActividadMetradoValor] = useState(0);
	async function onChangeInputActividadMetradoValor(actividad) {
		setInputActividadMetradoValor(actividad)
	}
	async function updateActividadesParcial() {
		console.log("data a enviar",{
			"id_actividad":InputActividadMetradoEstado,
			"metrado": InputActividadMetradoValor
		});
		const res = await axios.post(`${UrlServer}/updateActividadesParcial`,
		{
			"id_actividad":InputActividadMetradoEstado,
			"metrado": InputActividadMetradoValor,
			"id_partida":PartidaSelecccionado.id_partida
		}
		)
		alert("exito")
		fetchPartidas()
		fectchActividades(PartidaSelecccionado.id_partida)
		setInputActividadMetradoEstado(-1)
	}
														...
														<tbody>
															{
																Actividades.map((item) =>
																	<tr key={item.id_actividad} className={item.actividad_estado === "Mayor Metrado" ? 'FondMM icoVer' : 'icoVer'}>
																		...
																		<td>
																			{
																				InputActividadMetradoEstado== item.id_actividad ?
																					<div
																						className="d-flex"
																					>
																						<DebounceInput
																							...
																							debounceTimeout={300}
																							onChange={e => onChangeInputActividadMetradoValor(e.target.value)}
																							type="number"
																							size={"sm"}
																							style={{
																								width:"50%"
																						...
																						/>
																						<div
																							onClick={() => updateActividadesParcial()}
																						>
																							<MdSave style={{ cursor: "pointer" }} />
																						</div>
																						<div
																							onClick={() => toggleInputActividadMetradoEstado(-1)}
																						>
																					...
																					<div
																						className="d-flex"
																					>
																						{Redondea(item.parcial)} {PartidaSelecccionado.unidad_medida}
																						<div
																							onClick={() => toggleInputActividadMetradoEstado(item.id_actividad)}
																						>
