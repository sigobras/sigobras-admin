...
export default () => {
	...
	function calcularGrupo(grupo) {
		var grupo = ExcelRows.slice(cantidadPorGrupo * grupo, cantidadPorGrupo * (grupo + 1));
	...
	const [PartidasNoEncontradasZone, setPartidasNoEncontradasZone] = useState(false)
	function analizarPartidasNoEncontradas() {
		var noEncontrados = []
		for (let i = 0; i < Partidas.length; i++) {
			const partidaSistema = Partidas[i];
			var encontrado = ExcelRows.find(row =>
				removeLeadingZeros(row[ItemBasePosicion.col]) == removeLeadingZeros(partidaSistema.item)
			)
			if (!encontrado) {
	...
	const analizarPartidasDuplicadas = () => {
		let sorted_arr = Partidas.slice().sort(
			(a, b) => (a.item > b.item) ? 1 : ((b.item > a.item) ? -1 : 0)
		);
		let results = [];
	...
	function PartidasFiltradasComponente() {
		const result = ExcelRows.filter(
			(row, i) => {
				if (row[ItemBasePosicion.col] == null)
					return false
				var componente = row[ItemBasePosicion.col].substring(0, row[ItemBasePosicion.col].indexOf("."));
		...
		<div>
			...
			{PresupuestoZone &&
				<div>
														...
														{(
															() => {
																...
																element.forEach((td, j) => {
																	tds.push(
																		<td
																			onClick={() => onSelectParcial({ row: i, col: j })}
																			style={{
																				cursor: "pointer"
																			}}
																		>
					...
					<div>POSICION PRIMER ITEM PARTIDA :{ItemBasePosicion.row + "-" + ItemBasePosicion.col}</div>
...

			{
				...
				<Button
					color="danger"
					onClick={() => setPartidasNoEncontradasZone(!PartidasNoEncontradasZone)}>
				...
				<div className="row">
					<table className="table table-bordered table-sm table-hover">
						<thead>
							...
							<div>
										{(
											() => Redondea(PartidasFiltradas.reduce((acc, item) => {
												// console.log(item);
						...
						<tbody>
							{PartidasFiltradas.map((row, i) =>
								<tr>
									{row.map((celda, j) =>
										(j >= ItemBasePosicion.col && (j <= ItemBasePosicion.col + 5)) &&
										<td>{celda}</td>
									)}
									<CalcularMetrado
...
function CalcularMetrado({ row, itemPosicion, Partidas, ExcelRowsGrupo, setExcelRowsGrupo, rowPosicion, removeLeadingZeros }) {
	useEffect(() => {
		EncontrarPartida()
	}, [])
	...
	function EncontrarPartida() {
		var encontrado = Partidas.find(element =>
			removeLeadingZeros(element.item) == removeLeadingZeros(row[itemPosicion])
		)
		setPartidaEncontrada(encontrado)
		if (encontrado) {
			setPartidaEncontrada(encontrado)
			ExcelRowsGrupo[rowPosicion].parcial_sistema =
				(encontrado
					?
